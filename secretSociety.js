function secretName(names)
{
   ...
   /* 
      //Version con reduce 
   return arN.reduce((acc, cur) => (typeof cur === "string" && cur.length > 0)
      ? acc + cur[0].toUpperCase() : acc , '').split('').sort().join('');
   */
   return arN.map((cur, i, arr) => (typeof cur === "string" && cur.length > 0) 
